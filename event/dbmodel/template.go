...
func genTableNameFunc(model *Model, ignoreCreateTableNameFunc bool) error {
	data := TableNameTemplateParam{
		...
		TableNameStr:            convertUpLetterToUnderscorePlusLowLetter(model.TableName),
	}
...
import (
    ...
    "github.com/pingdai/tools/timelib"{{else if eq .CreateTimeFieldType "timelib.MySQLTimestamp"}}
    "github.com/pingdai/tools/timelib"{{end}}
...
func ({{.ReceiverName}} *{{.StructName}}) Create(db *gorm.DB) error {
    ...
    }{{else if eq .CreateTimeFieldType "timelib.MySQLTimestamp"}} if {{.ReceiverName}}.CreateTime.IsZero() {
        {{.ReceiverName}}.CreateTime = timelib.MySQLTimestamp(time.Now()).Time()
    }{{end}}
    {{end}}
    ...
    }{{else if eq .UpdateTimeFieldType "timelib.MySQLTimestamp"}} if {{.ReceiverName}}.UpdateTime.IsZero() {
        {{.ReceiverName}}.UpdateTime = timelib.MySQLTimestamp(time.Now()).Time() 
    }{{end}}
    {{end}}
    {{if .HasEnabledField}} 
    {{.ReceiverName}}.Enabled = {{.EnabledFieldType}}(true){{end}}
...
func ({{.ReceiverListName}} *{{.StructListName}}) FetchList(db *gorm.DB, size, offset int32, query ...map[string]interface{}) (int32, error) {
    ...
    if len(query) == 0 {
        query = append(query, map[string]interface{}{"{{.DbEnabledField}}": true})
    } else {
        if _, ok := query[0]["{{.DbEnabledField}}"]; !ok { 
            query[0]["{{.DbEnabledField}}"] = true 
        }
    }
    ...
    var err error	
	ret := db.Table({{.StructName}}{}.TableName()).Where(query[0])
	err = ret.Count(&count).Error
	if err != nil {
		return count, err
	}

    {{if .HasCreateTimeField}}err = ret.Limit(size).Offset(offset).Order("{{.DbCreateTimeField}} desc").Find({{.ReceiverListName}}).Error{{else}}err = ret.Limit(size).Offset(offset).Find({{.ReceiverListName}}).Error{{end}}
	{{else}}
    if size <= 0 {
        size = -1
        offset = -1
    ...
    var err error
	ret := db.Table({{.StructName}}{}.TableName())
	err = ret.Count(&count).Error
	if err != nil {
		return count, err
	}
    ...
    if len(query) == 0 {
        ...
        {{if .HasCreateTimeField}}err = ret.Where(query[0]).Limit(size).Offset(offset).Order("{{.DbCreateTimeField}} desc").Find({{.ReceiverListName}}).Error{{else}}err = ret.Where(query[0]).Limit(size).Offset(offset).Find({{.ReceiverListName}}).Error{{end}}
    }{{end}}
...
func ({{.ReceiverListName}} *{{.StructListName}}) BatchFetchBy{{.Field}}List(db *gorm.DB, {{.ParamField}}List []{{.FieldType}}) error {
    ...
    {{if .HasEnabledField}}err := db.Table({{.StructName}}{}.TableName()).Where("{{.DbField}} in (?) and {{.DbEnabledField}} = ?", {{.ParamField}}List, true).Find({{.ReceiverListName}}).Error{{else}}
...
func ({{.ReceiverListName}} *{{.StructListName}}) FetchBy{{.PartFuncName}}({{.FuncInputParam}}) error {
	{{if .HasEnabledField}}err := db.Table({{.StructName}}{}.TableName()).Where("{{.OrmQueryFormat}} and {{.DbEnabledField}} = ?", {{.OrmQueryParam}}, true).Find({{.ReceiverListName}}).Error{{else}} 
...
func ({{.ReceiverName}} *{{.StructName}}) UpdateBy{{.PartFuncName}}WithStruct({{.FuncInputParam}}) error {
    ...
    {{end}}
    {{if .HasEnabledField}}dbRet := db.Table({{.ReceiverName}}.TableName()).Where("{{.OrmQueryFormat}} and {{.DbEnabledField}} = ?", {{.OrmQueryParam}}, true).Updates({{.ReceiverName}}){{else}} 
    ...
    if err != nil {
        ...
        if dbRet.RowsAffected == 0 {
            {{if .HasEnabledField}}findErr := db.Table({{.ReceiverName}}.TableName()).Where("{{.OrmQueryFormat}} and {{.DbEnabledField}} = ?", {{.OrmQueryParam}}, true).Find(&{{.StructName}}{}).Error{{else}}
...
func ({{.ReceiverName}} *{{.StructName}}) UpdateBy{{.PartFuncName}}WithMap({{.FuncInputParam}}, updateMap map[string]interface{}) error {
    {{if .HasUpdateTimeField}}if _, ok := updateMap["{{.DbUpdateTimeField}}"]; !ok { 
		...
		{{else if eq .UpdateTimeFieldType "timelib.MySQLTimestamp"}}updateMap["{{.DbUpdateTimeField}}"] = time.Time(timelib.MySQLTimestamp(time.Now()).Time())
        {{end}}
    }{{end}}
    {{if .HasEnabledField}}dbRet := db.Table({{.ReceiverName}}.TableName()).Where("{{.OrmQueryFormat}} and {{.DbEnabledField}} = ?", {{.OrmQueryParam}}, true).Updates(updateMap){{else}} 
...
func ({{.ReceiverName}} *{{.StructName}}) SoftDeleteBy{{.PartFuncName}}({{.FuncInputParam}}) (err error) {
    {{if .HasEnabledField}}var updateMap = map[string]interface{}{}
    updateMap["{{.DbEnabledField}}"] = false
    {{if .HasUpdateTimeField}}
    {{if eq .UpdateTimeFieldType "int32" "int64" "uint32" "uint64"}}if {{.ReceiverName}}.UpdateTime == 0 { 
        {{.ReceiverName}}.UpdateTime = time.Now().Unix() 
    }{{else if eq .UpdateTimeFieldType "time.Time"}}if {{.ReceiverName}}.UpdateTime.IsZero() {
    ...
    {{end}}
    err = db.Table({{.ReceiverName}}.TableName()).Where("{{.OrmQueryFormat}} and {{.DbEnabledField}} = ?", {{.OrmQueryParam}}, true).Updates(updateMap).Error
...
func ({{.ReceiverName}} *{{.StructName}}) DeleteBy{{.PartFuncName}}({{.FuncInputParam}}) (err error) {
    {{if .HasEnabledField}}err = db.Table({{.ReceiverName}}.TableName()).Where("{{.OrmQueryFormat}} and {{.DbEnabledField}} = ?", {{.OrmQueryParam}}, true).Delete({{.ReceiverName}}).Error{{else}}
...
func ({{.ReceiverName}} *{{.StructName}}) FetchBy{{.PartFuncName}}ForUpdate({{.FuncInputParam}}) error {
    {{if .HasEnabledField}}err := db.Table({{.ReceiverName}}.TableName()).Where("{{.OrmQueryFormat}} and {{.DbEnabledField}} = ?", {{.OrmQueryParam}}, true).Set("gorm:query_option", "FOR UPDATE").Find({{.ReceiverName}}).Error{{else}} 
...
func ({{.ReceiverName}} *{{.StructName}}) FetchBy{{.PartFuncName}}({{.FuncInputParam}}) error {
    {{if .HasEnabledField}}err := db.Table({{.ReceiverName}}.TableName()).Where("{{.OrmQueryFormat}} and {{.DbEnabledField}} = ?", {{.OrmQueryParam}}, true).Find({{.ReceiverName}}).Error{{else}} 
