...
// Source：https://github.com/fvbock/endless
// 官方推荐：https://github.com/gin-gonic/gin#graceful-restart-or-stop
...
func NewServer(addr string, handler http.Handler) (srv *endlessServer) {
	...
	srv.BeforeBegin = func(addr string) {
		logrus.Println(syscall.Getpid(), addr)
	}
...
func (srv *endlessServer) Serve() (err error) {
	defer logrus.Println(syscall.Getpid(), "Serve() returning...")
	srv.setState(STATE_RUNNING)
	err = srv.Server.Serve(srv.EndlessListener)
	logrus.Println(syscall.Getpid(), "Waiting for connections to finish...")
	srv.wg.Wait()
...
func (srv *endlessServer) ListenAndServeTLS(certFile, keyFile string) (err error) {
	...
	logrus.Println(syscall.Getpid(), srv.Addr)
	return srv.Serve()
}
...
func (srv *endlessServer) handleSignals() {
	...
	for {
		...
		switch sig {
		case syscall.SIGHUP:
			logrus.Println(pid, "Received SIGHUP. forking.")
			err := srv.fork()
			if err != nil {
				logrus.Println("Fork err:", err)
			}
		case syscall.SIGINT:
			logrus.Println(pid, "Received SIGINT.")
			srv.shutdown()
		case syscall.SIGQUIT:
			logrus.Println(pid, "Received SIGINT.")
			srv.shutdown()
		case syscall.SIGTERM:
			logrus.Println(pid, "Received SIGTERM.")
			srv.shutdown()
		default:
			logrus.Printf("Received %v: nothing i care about...\n", sig)
		}
...
func (srv *endlessServer) shutdown() {
	...
	if err != nil {
		logrus.Println(syscall.Getpid(), "Listener.Close() error:", err)
	} else {
		logrus.Println(syscall.Getpid(), srv.EndlessListener.Addr(), "Listener closed.")
	}
}
...
func (srv *endlessServer) hammerTime(d time.Duration) {
	defer func() {
		...
		if r := recover(); r != nil {
			logrus.Println("WaitGroup at 0", r)
		}
	}()
	...
	time.Sleep(d)
	logrus.Println("[STOP - Hammer Time] Forcefully shutting down parent")
	for {
